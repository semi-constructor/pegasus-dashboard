...
* **Subcommands:**
  * `view`: View audit logs.
    * `[user]` (User) **[Optional]**: Filter logs by a specific user.
    * `[action_type]` (String) **[Optional]**: Filter logs by action type (e.g., `BAN`, `KICK`, `TIMEOUT`).
    * `[limit]` (Integer) **[Optional]**: Number of logs to fetch (1-50, default: 10).

### `/blacklist`
Manage the bot's global user blacklist (prevents blacklisted users from using any commands).
* **Subcommands:**
  * `add`: Add a user to the blacklist.
    * `<user>` (User) **[Required]**: The user to blacklist.
    * `[reason]` (String) **[Optional]**: Reason for blacklisting.
  * `remove`: Remove a user from the blacklist.
    * `<user>` (User) **[Required]**: The user to unblacklist.
...
* **Subcommands:**
  * `enable`: Enable a bot module.
    * `<module>` (String) **[Required]**: Choices: `economy`, `tickets`, `fun`, `moderation`, `xp`, `giveaways`, `jtc`, `automod`.
...
* **Options:**
  ...
  * `[dry_run]` (Boolean) **[Optional]**: If true, simulates the prune and returns the count without kicking members.
  * `[roles]` (Role) **[Optional]**: Only prune members with this specific role.
  * `[reason]` (String) **[Optional]**: Audit log reason for the prune.
...
* **Subcommands:**
  ...
  * `set`: Set a specific configuration key.
    * `<setting>` (String) **[Required]**: Setting to change. Choices: `prefix`, `log_channel`, `welcome_channel`, `mute_role`, `max_warnings`.
    * `<value>` (String) **[Required]**: The new value (Channel ID, Role ID, or text/number depending on the setting).

### `/embed`
Create and send fully customizable rich embeds.
* **Subcommands:**
  * `create`: Create a custom embed.
    * `<channel>` (Channel) **[Required]**: The channel where the embed will be sent (Text/Announcement channels).
    * `<title>` (String) **[Required]**: Embed title (max 256 characters).
    * `<description>` (String) **[Required]**: Embed description (max 4096 characters).
    * `[color]` (String) **[Optional]**: Hex color code (e.g., `#5865F2`).
    * `[url]` (String) **[Optional]**: Title clickable URL.
    * `[author_name]` (String) **[Optional]**: Author text.
    * `[author_icon]` (String) **[Optional]**: Author icon image URL.
    * `[thumbnail]` (String) **[Optional]**: Thumbnail image URL.
    * `[image]` (String) **[Optional]**: Main large image URL.
    * `[footer_text]` (String) **[Optional]**: Footer text.
    * `[footer_icon]` (String) **[Optional]**: Footer icon image URL.
    * `[timestamp]` (Boolean) **[Optional]**: Whether to include the current timestamp.

### `/reactionrole`
Manage reaction-based role assignment.
* **Subcommands:**
  * `add`: Add a reaction role to a message.
    * `<message_id>` (String) **[Required]**: The target message ID.
    * `<emoji>` (String) **[Required]**: The emoji to react with.
    * `<role>` (Role) **[Required]**: The role to assign upon reacting.
    * `[channel]` (Channel) **[Optional]**: The channel where the message is located.
  ...
  * `list`: List all configured reaction roles.
    * `[channel]` (Channel) **[Optional]**: Filter reaction roles by a specific channel.
...
* **Subcommands & Groups:**
  * `balance`: View current coin balance.
    * `[user]` (User) **[Optional]**: The user whose balance you want to check.
  ...
  * `rob`: Attempt to steal coins from another user.
    * `<user>` (User) **[Required]**: The target user to rob.
  * **`gamble` (Subcommand Group):**
    * `dice`: Roll a dice to double or lose your bet.
      * `<amount>` (Integer) **[Required]**: Amount of coins to bet.
    * `flip`: Flip a coin (heads/tails).
      * `<amount>` (Integer) **[Required]**: Amount of coins to bet.
      * `<choice>` (String) **[Required]**: Choices: `Heads`, `Tails`.
    * `slots`: Play the slot machine.
      * `<amount>` (Integer) **[Required]**: Amount of coins to bet.
  * **`shop` (Subcommand Group):**
    * `list`: List all available items in the server shop.
    * `buy`: Buy an item from the shop.
      * `<item_id>` (String) **[Required]**: The ID of the item to purchase.
    ...
    * `use`: Use an item from your inventory.
      * `<item_id>` (String) **[Required]**: The ID of the item to use.
    * `inventory`: View item inventory.
      * `[user]` (User) **[Optional]**: The user whose inventory you want to check.
...
* **Subcommands:**
  * `start`: Start a giveaway with an advanced interactive modal (allows setting description, requirements, and bonus entries).
    * `<prize>` (String) **[Required]**: The prize description (max 255 characters).
    * `<duration>` (String) **[Required]**: Duration (e.g., `10m`, `2h`, `1d`).
    * `[winners]` (Integer) **[Optional]**: Number of winners (1-50, default: 1).
    * `[channel]` (Channel) **[Optional]**: Text channel to host the giveaway in.
    * `[embed_title]` (String) **[Optional]**: Custom embed title.
    * `[embed_image]` (Attachment) **[Optional]**: Custom embed image upload.
    * `[embed_thumbnail]` (Attachment) **[Optional]**: Custom embed thumbnail upload.
  * `simple`: Quickly start a simple giveaway directly in the current channel.
    * `<prize>` (String) **[Required]**: The prize description.
    * `<duration>` (String) **[Required]**: Duration (e.g., `10m`, `2h`, `1d`).
    * `<winners>` (Integer) **[Required]**: Number of winners (1-50).
    * `[embed_title]` (String) **[Optional]**: Custom embed title.
    * `[embed_image]` (Attachment) **[Optional]**: Custom embed image upload.
    * `[embed_thumbnail]` (Attachment) **[Optional]**: Custom embed thumbnail upload.
  * `end`: Immediately end an active giveaway and roll winners.
    * `<giveaway_id>` (String) **[Required]**: The message ID / giveaway ID.
  * `reroll`: Reroll winners for a finished giveaway.
    * `<giveaway_id>` (String) **[Required]**: The message ID / giveaway ID.
    * `[winners]` (Integer) **[Optional]**: Number of new winners to select.
...
* **Subcommands:**
  * `add_rule`: Add a new automated rule.
    ...
    * `<trigger_type>` (String) **[Required]**: Trigger condition. Choices: `Keyword Match`, `Regex Match`, `Mention Spam`, `Attachment Spam`.
    * `<action_type>` (String) **[Required]**: Action to execute. Choices: `Delete Message`, `Warn User`, `Timeout User`, `Add Infraction Points`.
    * `[keywords]` (String) **[Optional]**: Comma-separated keywords (required if trigger is Keyword).
    * `[regex_pattern]` (String) **[Optional]**: Regex pattern (required if trigger is Regex).
    * `[limit]` (Integer) **[Optional]**: Threshold limit for mention/attachment spam.
    * `[points]` (Integer) **[Optional]**: Infraction points to add (if action is Add Infraction Points).
  * `list_rules`: List all configured AutoMod V2 rules in the server.
  * `remove_rule`: Delete an AutoMod V2 rule.
    * `<rule_id>` (String) **[Required]**: UUID of the rule to remove.
  ...
  * `quarantine_release`: Release a user from the Quarantine Vault.
    * `<user>` (User) **[Required]**: The user to release.

### `/filter`
Manage automated word filtering rules.
* **Subcommands:**
  * `add`: Add a word filter rule.
    * `<pattern>` (String) **[Required]**: The word or regex pattern to filter.
    * `[match_type]` (String) **[Optional]**: Choices: `Literal`, `Regex`.
    * `[case_sensitive]` (Boolean) **[Optional]**: Whether matching should be case-sensitive.
    * `[whole_word]` (Boolean) **[Optional]**: Whether to match only whole words.
    * `[severity]` (String) **[Optional]**: Choices: `Low`, `Medium`, `High`, `Critical`.
    * `[auto_delete]` (Boolean) **[Optional]**: Whether to automatically delete matching messages.
    * `[notify_channel]` (Channel) **[Optional]**: Target channel for filter violation alerts.
  * `remove`: Remove a word filter rule.
    * `<rule_id>` (Integer) **[Required]**: ID of the filter rule.
...
* **Subcommands & Groups:**
  * `ban`: Ban a user from the server.
    ...
    * `[delete_days]` (Integer) **[Optional]**: Number of days of messages to delete (0-7).
  ...
  * `timeout`: Temporarily timeout (mute) a user.
    ...
    * `<duration>` (Integer) **[Required]**: Choices range from `60 seconds` to `4 weeks`.
    * `[reason]` (String) **[Optional]**: Reason for timeout.
  * `mute`: Assign the server mute role to a user.
    * `<user>` (User) **[Required]**: User to mute.
    * `[duration]` (Integer) **[Optional]**: Duration in minutes (1-10080).
  ...
  * `purge`: Delete multiple messages rapidly.
    * `<amount>` (Integer) **[Required]**: Number of messages to delete (2-100).
    * `[user]` (User) **[Optional]**: Only delete messages sent by this user.
    * `[channel]` (Channel) **[Optional]**: Target channel to purge in.
  * `lock`: Lock a channel (prevents regular members from sending messages).
    * `[channel]` (Channel) **[Optional]**: Target channel to lock.
    * `[reason]` (String) **[Optional]**: Reason for locking.
  * `unlock`: Unlock a locked channel.
    * `[channel]` (Channel) **[Optional]**: Target channel to unlock.
    * `[reason]` (String) **[Optional]**: Reason for unlocking.
  * `slowmode`: Set slowmode delay in a channel.
    * `<duration>` (Integer) **[Required]**: Slowmode delay in seconds (0-21600). `0` disables slowmode.
    * `[channel]` (Channel) **[Optional]**: Target channel.
    * `[reason]` (String) **[Optional]**: Reason for slowmode change.
  * `modlog`: View moderation history logs.
    * `[user]` (User) **[Optional]**: Filter logs by user.
    * `[limit]` (Integer) **[Optional]**: Number of logs to show (1-25).
...
* **Subcommands & Groups:**
  * `create`: Issue a warning to a user.
    ...
    * `<title>` (String) **[Required]**: Summary/title of the warning.
    * `[description]` (String) **[Optional]**: Detailed description/context.
    * `[level]` (Integer) **[Optional]**: Severity level (1-10).
    * `[proof]` (Attachment) **[Optional]**: Supporting screenshot/attachment.
  * `edit`: Edit an existing warning.
    * `<warnid>` (String) **[Required]**: Warning ID to edit.
  * `lookup`: Look up details of a specific warning.
    * `<warnid>` (String) **[Required]**: Warning ID.
  ...
  * **`automation` (Subcommand Group):**
    * `create`: Create an automation trigger (e.g., timeout/ban when warn count or level is reached).
      * `<trigger_type>` (String) **[Required]**: Choices: `Warn Count`, `Warn Level`.
      * `<trigger_value>` (Integer) **[Required]**: Threshold number that activates the trigger (1-100).
      * `[notify_channel]` (Channel) **[Optional]**: Channel to send alert when automation triggers.
...
* **Subcommands & Groups:**
  ...
  * `close`: Close an active ticket.
    * `[reason]` (String) **[Optional]**: Reason for closing the ticket.
  * `stats`: View ticket system statistics for the server.
  * **`panel` (Subcommand Group):**
    * `create`: Create a new ticket panel configuration.
      * `<panel_id>` (String) **[Required]**: Unique ID for the panel (3-20 chars).
      * `<title>` (String) **[Required]**: Panel title.
      * `<description>` (String) **[Required]**: Panel description.
      * `[button_label]` (String) **[Optional]**: Text on the open ticket button.
      * `[button_style]` (Integer) **[Optional]**: Choices: `Primary (Blue)`, `Secondary (Gray)`, `Success (Green)`, `Danger (Red)`.
      * `[category]` (Channel) **[Optional]**: Discord Category where tickets will be created.
      * `[support_role]` (Role) **[Optional]**: Staff role granted access to tickets.
      * `[max_tickets]` (Integer) **[Optional]**: Max concurrent tickets per user (1-10).
      * `[welcome_message]` (String) **[Optional]**: Initial message sent when a ticket opens.
    * `load`: Send an existing ticket panel to a specific channel.
      * `<panel_id>` (String) **[Required]**: ID of the panel to load.
      * `<channel>` (Channel) **[Required]**: Text channel to post the panel in.
    * `delete`: Delete a ticket panel configuration.
      * `<panel_id>` (String) **[Required]**: ID of the panel to delete.
    * `list`: List all configured ticket panels in the server.
    * `edit`: Open interactive editor for a ticket panel.
      * `<panel_id>` (String) **[Required]**: ID of the panel to edit.
    * `add_dept`: Add a specific department choice to a ticket panel.
      * `<panel_id>` (String) **[Required]**: Target panel ID.
      * `<dept_id>` (String) **[Required]**: Unique department ID.
      * `<name>` (String) **[Required]**: Department display name.
      * `<description>` (String) **[Required]**: Department description.
      * `[support_role]` (Role) **[Optional]**: Specific support role for this department.
      * `[category]` (Channel) **[Optional]**: Specific category channel for this department.
      * `[welcome_message]` (String) **[Optional]**: Specific welcome message for this department.
    * `list_depts`: List all departments configured on a panel.
      * `<panel_id>` (String) **[Required]**: Target panel ID.
    * `remove_dept`: Remove a department from a panel.
      * `<panel_id>` (String) **[Required]**: Target panel ID.
      * `<dept_id>` (String) **[Required]**: Department ID to remove.
...
* **Subcommands:**
  * `setup`: Initial configuration for JTC.
    * `<base_voice>` (Channel) **[Required]**: The master voice channel users join to create their own room.
    * `<category>` (Channel) **[Required]**: Category where dynamic voice channels will be created.
    * `<panel_channel>` (Channel) **[Required]**: Text channel where the JTC management panel will be sent.
    * `[name_format]` (String) **[Optional]**: Custom naming template for created channels.
...
* **Subcommands:**
  ...
  * `set`: Change your language preference.
    * `<language>` (String) **[Required]**: Choices: `English`, `Deutsch`, `Español`, `Français`.
...
* **Subcommands:**
  ...
  * `steam`: Look up a Steam user profile.
    * `<username>` (String) **[Required]**: Steam username or custom profile URL.
  ...
  * `help`: View interactive command help.
    * `[command]` (String) **[Optional]**: Specific command name (supports autocomplete).
...
* **Options:**
  * `<user>` (User) **[Required]**: The user to thank.
  * `[reason]` (String) **[Optional]**: Reason for thanking them.
...
* **Subcommands:**
  ...
  * `leaderboard`: View the server's top XP leaderboard.
    * `[page]` (Integer) **[Optional]**: Page number to view.
