...
export default async function AdminSurveyStatsPage({ params }: { params: Promise<{ surveyId: string }> }) {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground">Admin overview</p>
        </div>
        <div className="flex gap-2">
          <CopyLinkButton surveyId={survey.id} />
          <Link href="/dashboard/admin/surveys">
            <Button variant="outline">Back to Surveys</Button>
          </Link>
          <DeleteSurveyButton surveyId={survey.id} scope="SYSTEM" guildId={null} />
        </div>
      </div>
