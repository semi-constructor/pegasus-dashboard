...
export default function JtcClient({
 ...
 await saveJtcConfig(guildId, config);
      ...
      <div className="flex items-center justify-between border-b border-foreground/10 pb-6 mb-8">
        <div>
          <h1 className="text-3xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-br from-foreground to-foreground/60 tracking-tight flex items-center gap-4">
            <div className="p-3 bg-foreground/5 rounded-2xl border border-foreground/10 backdrop-blur-md">
  ...
  <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">{t("config.baseChannel")}</label>
  <DiscordChannelPicker
  ...
  />
  <p className="text-xs text-foreground/50">{t("config.baseChannelDesc")}</p>
  </div>

  <div className="flex flex-col gap-2">
  <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">{t("config.category")}</label>
  <DiscordChannelPicker
  ...
  />
  <p className="text-xs text-foreground/50">{t("config.categoryDesc")}</p>
  </div>

  <div className="flex flex-col gap-2">
  <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">{t("config.panelChannel")}</label>
  <DiscordChannelPicker
  ...
  />
  <p className="text-xs text-foreground/50">{t("config.panelChannelDesc")}</p>
  </div>

  <div className="flex flex-col gap-2">
  <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">{t("config.nameFormat")}</label>
  <Input
  ...
  />
  <p className="text-xs text-foreground/50 uppercase">{t("config.nameFormatDesc", { user: '{user}' })}</p>
  </div>
  </div>
  ...
  onClick={handleSave}
  disabled={isPending}
  className="bg-foreground/10 hover:bg-foreground/20 text-foreground border-0 shadow-sm font-bold text-xs uppercase mt-4"
  >
  ...
  <p className="text-foreground/40 text-sm uppercase p-4 border border-foreground/10 bg-foreground/5 rounded-xl">
  {t("active.noData")}
  </p>
  ) : (
  ...
  <div
  key={ch.id}
  className="p-4 rounded-xl bg-black/20 border border-foreground/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm"
  >
  <div>
