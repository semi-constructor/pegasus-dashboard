...
export default function ScheduleClient({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-white/60 tracking-tight flex items-center gap-4">
          <div className="p-3 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md">
            <Calendar className="w-8 h-8 text-indigo-400" />
          </div>
          {t('title')}
        </h1>
        <p className="text-white/40 mt-3 text-sm">{t('description')}</p>
      </div>

      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-white/5 border border-white/10 rounded-2xl shadow-2xl relative overflow-hidden flex flex-col backdrop-blur-md"
      >
        ...
        <form onSubmit={handleSchedule} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">{t('prize')}</label>
              <input
                type="text"
                value={prize}
                onChange={(e) => setPrize(e.target.value)}
                placeholder="e.g. 1 Month Nitro"
                className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-sm focus:outline-none focus:border-indigo-500/50 transition-colors"
                required
              />
            </div>
            
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">{t('channel')}</label>
              <select
                value={channelId}
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="text-sm font-medium text-white/80">{t('desc')}</label>
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Optional details about the giveaway..."
                className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-sm focus:outline-none focus:border-indigo-500/50 transition-colors h-24"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-sm focus:outline-none focus:border-indigo-500/50 transition-colors [color-scheme:dark]"
                required
              />
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">{t('startTime')}</label>
              <input
                type="time"
                value={startTime}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">{t('duration')}</label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">{t('winners')}</label>
              <input
                type="number"
                min="1"
                max="20"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-medium py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(99,102,241,0.2)] hover:shadow-[0_0_30px_rgba(99,102,241,0.4)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
        ...
        <form onSubmit={handleScheduleTrivia} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">{t('type') || 'Type'}</label>
              <select
                value={triviaType}
                onChange={(e) => setTriviaType(e.target.value as any)}
                className="w-full min-h-[40px] px-3 py-2 bg-black/40 border border-white/10 rounded-lg text-sm text-white [&>option]:bg-neutral-900 outline-none focus-visible:ring-1 focus-visible:ring-primary/50 transition-all"
              >
            ...
            {triviaType === "preset" ? (
              <>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-white/80">{t('preset') || 'Preset Category'}</label>
                  <select
                    value={preset}
                ...
                <div className="space-y-2 md:col-span-2">
                  <label className="text-sm font-medium text-white/80">{t('questionCount') || 'Question Count'} (Max 25)</label>
                  <input
                    type="number"
                    min="1"
                    max="25"
              ...
              <div className="md:col-span-2 space-y-4">
                {customQuestions.map((q, qIndex) => (
                  <div key={qIndex} className="p-4 border border-white/10 rounded-xl space-y-4 bg-white/5 relative">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-white/80">Question {qIndex + 1}</label>
                      <input
                        type="text"
                        value={q.question}
                        onChange={(e) => {
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {q.options.map((opt, optIndex) => (
                        <div key={optIndex} className="space-y-2">
                          <label className="text-sm font-medium text-white/80">Option {optIndex + 1}</label>
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              className="w-full bg-black/40 border border-white/10 rounded-lg p-2 text-sm focus:outline-none focus:border-indigo-500/50 transition-colors"
                              required
                            />
                          </div>
                        </div>
                ...
                {customQuestions.length < 25 && (
                  <button
                    ...
                    className="w-full border border-dashed border-white/20 p-3 rounded-xl text-sm font-medium text-white/60 hover:text-white hover:border-white/40 hover:bg-white/5 transition-all"
                  >
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">Reward XP</label>
              <input
                type="number"
                min="0"
                value={rewardXp}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">Reward Coins</label>
              <input
                type="number"
                min="0"
                value={rewardCoins}
