    ...
    <div className="text-foreground p-6 lg:p-10 max-w-[1400px] mx-auto animate-in fade-in duration-500 space-y-6 lg:space-y-8">
      ...
      >
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">Message Type</label>
            <select
              value={msgType}
              onChange={(e) => setMsgType(e.target.value)}
              className="w-full p-3 bg-background/40 border border-border rounded-lg text-sm uppercase text-foreground outline-none focus-visible:ring-1 focus-visible:ring-primary/50 transition-all"
            >
        ...
        {msgType === "text" ? (
          <div className="flex flex-col gap-2">
            <Textarea
              ...
              className="w-full bg-foreground/5 border border-border text-foreground placeholder:text-foreground/30 rounded-lg px-4 py-3 focus-visible:ring-1 focus-visible:ring-primary/50 transition-all min-h-[200px]"
              placeholder="Type your message here..."
            />
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 border border-border rounded-xl bg-foreground/5">
            <div className="flex flex-col gap-2">
              ...
              <Input 
                ...
                className="w-full bg-background/40 border border-border text-foreground placeholder:text-foreground/30 rounded-lg px-4 py-2"
                placeholder="Announcement"
              />
            </div>
            <div className="flex flex-col gap-2">
              <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">Embed Color (Hex)</label>
              <div className="flex gap-2">
                ...
                <Input 
                  value={embedData.color}
                  onChange={(e) => setEmbedData({ ...embedData, color: e.target.value })}
                  className="w-full bg-background/40 border border-border text-foreground placeholder:text-foreground/30 rounded-lg px-4 py-2 uppercase"
                  placeholder="#3498db"
                />
              </div>
            </div>
            <div className="space-y-1 md:col-span-2">
              ...
              <Textarea 
                ...
                className="w-full bg-background/40 border border-border text-foreground placeholder:text-foreground/30 rounded-lg px-4 py-3 min-h-[120px]"
                placeholder="Main embed content goes here..."
              />
            </div>
            <div className="flex flex-col gap-2">
              <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">Footer Text</label>
              <Input 
            ...
            <div className="flex flex-col gap-2">
              <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">Thumbnail URL</label>
              <Input 
            ...
            <div className="space-y-1 md:col-span-2">
              <label className="block text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2 ml-1">Image URL</label>
              <Input 
      ...
      <div className="flex justify-end mt-8">
        <Button 
          onClick={handleSend}
          disabled={isPending}
          className="bg-blue-600 hover:bg-blue-700 text-foreground px-8 py-6 h-auto text-lg font-bold flex items-center gap-3 rounded-xl transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)]"
        >
