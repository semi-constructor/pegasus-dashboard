  ...
  return (
    <div className="p-4 sm:p-8 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/dashboard/${params.guildId}/tickets`} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-white/70 hover:text-white">
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden p-6 space-y-4">
        {messages.length === 0 ? (
          ...
          messages.map((msg: any) => (
            <div key={msg.id} className="flex gap-4 p-4 rounded-lg bg-black/20 border border-white/5">
              ...
              <div className="flex-1">
                <div className="flex items-baseline gap-2 mb-1">
                  <span className="font-bold text-white/90">{msg.authorUsername || msg.authorId}</span>
                  <span className="text-xs text-white/40">{new Date(msg.createdAt).toLocaleString()}</span>
                </div>
