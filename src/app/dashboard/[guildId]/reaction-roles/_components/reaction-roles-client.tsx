...
export default function ReactionRolesClient({ guildId, channels, roles }: { guildId: string, channels: ChannelOption[], roles: DiscordRole[] }) {
  ...
  const [buttons, setButtons] = useState<RoleButton[]>([]);
    ...
    <div className="text-white p-6 lg:p-10 max-w-[1400px] mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              ...
              <Input 
                ...
                className="w-full bg-white/5 border border-white/10 text-white placeholder:text-white/30 rounded-lg px-4 py-2"
                placeholder="Self-Assign Roles"
              />
            </div>
            <div className="flex flex-col gap-2">
              <label className="block text-xs font-bold text-white/70 uppercase tracking-wider mb-2 ml-1">Embed Color</label>
              <div className="flex gap-2">
                ...
                <Input 
                  value={embedData.color}
                  onChange={(e) => setEmbedData({ ...embedData, color: e.target.value })}
                  className="w-full bg-white/5 border border-white/10 text-white placeholder:text-white/30 rounded-lg px-4 py-2 uppercase"
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <Textarea 
              ...
              className="w-full bg-white/5 border border-white/10 text-white placeholder:text-white/30 rounded-lg px-4 py-3 min-h-[100px]"
      ...
      >
        <div className="space-y-4">
          {buttons.length === 0 ? (
            <div className="border border-white/10 bg-white/5 rounded-xl p-8 text-center text-white/50 uppercase font-bold text-sm">
              No buttons added yet. Click "Add Button" to start.
            </div>
          ) : (
            buttons.map((btn, index) => (
              <div key={btn.id} className="p-4 bg-black/40 border border-white/10 rounded-xl relative group hover:border-primary/50 transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                  <div className="md:col-span-4 flex flex-col gap-2">
                    <label className="text-xs font-bold text-white/70 uppercase tracking-wider">Role</label>
                    <select
                      value={btn.roleId}
                      onChange={(e) => handleUpdateButton(btn.id, "roleId", e.target.value)}
                      className="w-full min-h-[40px] px-3 py-2 bg-black/40 border border-white/10 rounded-lg text-sm text-white [&>option]:bg-neutral-900 outline-none"
                    >
                  ...
                  <div className="md:col-span-3 flex flex-col gap-2">
                    <label className="text-xs font-bold text-white/70 uppercase tracking-wider">Button Label</label>
                    <Input
                      value={btn.label}
                      onChange={(e) => handleUpdateButton(btn.id, "label", e.target.value)}
                      className="w-full bg-white/5 border border-white/10 text-white placeholder:text-white/30"
                      placeholder="e.g. Announcements"
                    />
                  </div>

                  <div className="md:col-span-2 flex flex-col gap-2">
                    <label className="text-xs font-bold text-white/70 uppercase tracking-wider">Emoji</label>
                    <Input
                      value={btn.emoji}
                  ...
                  <div className="md:col-span-2 flex flex-col gap-2">
                    <label className="text-xs font-bold text-white/70 uppercase tracking-wider">Style</label>
                    <select
                      value={btn.style}
