...
import { Coins, Shield, Users, ChevronRight } from "lucide-react";
        ...
        <div className="max-w-5xl mx-auto px-6 lg:px-24 relative z-10">
          <div className="mb-24">
            <div className="inline-flex items-center gap-2 text-foreground/30 text-xs tracking-[0.3em] uppercase mb-8 border border-border px-4 py-2">
              <Coins className="w-3 h-3" /> GLOBAL_ECONOMY
            </div>
          ...
          <div className="border border-border bg-[#050505]">
            {topGuilds.length === 0 ? (
              <div className="text-center py-24">
                <Shield className="w-8 h-8 text-foreground/20 mx-auto mb-4" />
                <h3 className="text-sm font-medium text-foreground/50 uppercase tracking-[0.2em] mb-2">No Servers Found</h3>
                <p className="text-foreground/30 text-xs uppercase tracking-[0.2em]">Be the first to enable Public Economy and claim the #1 spot.</p>
              </div>
            ) : (
              <div className="divide-y divide-white/5">
                {topGuilds.map((guild, index) => (
                  <Link 
                    href={`/eco/${guild.id}`} 
                    key={guild.id}
                    className="flex items-center gap-6 p-6 hover:bg-foreground/[0.02] transition-all group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-medium text-foreground uppercase tracking-[0.1em] truncate">
                        Server {guild.id}
                      </h3>
                      <div className="flex items-center gap-2 text-[10px] text-foreground/30 uppercase tracking-[0.3em] mt-1">
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <div className="text-[10px] text-foreground/20 uppercase tracking-[0.3em]">Total Volume</div>
                    </div>

                    <ChevronRight className="w-4 h-4 text-foreground/10 group-hover:text-foreground group-hover:translate-x-1 transition-all flex-shrink-0 hidden sm:block" />
                  </Link>
                ))}
              </div>
            )}
