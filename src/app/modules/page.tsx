            ...
            {modulesList.map((mod, i) => (
              ...
              >
                <Link href={`/module/${mod.name}`} className="block h-full">
                  <div className="group bg-[#050505] p-8 md:p-10 hover:bg-foreground/[0.02] transition-all duration-500 h-full flex flex-col relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-px bg-foreground/0 group-hover:bg-foreground/20 transition-all duration-700" />
                    
                    <div className="flex items-center justify-between mb-8">
                      <mod.icon className="w-6 h-6 text-foreground/30 group-hover:text-foreground transition-colors duration-500" />
                      <span className="text-[10px] font-mono uppercase tracking-[0.3em] text-foreground/20">
                        {String(i + 1).padStart(2, '0')}
                      </span>
                    </div>
                    
                    <h3 className="text-xl font-medium text-foreground mb-4 uppercase tracking-[0.1em] group-hover:tracking-[0.15em] transition-all duration-500">
                      {t(`modules.${mod.name}.title`)}
                    </h3>
                    
                    <p className="text-foreground/40 text-sm leading-relaxed font-light flex-grow">
                      {t(`modules.${mod.name}.description`)}
                    </p>

                    <div className="mt-8 flex items-center text-xs uppercase tracking-[0.3em] text-foreground/20 group-hover:text-foreground transition-colors duration-500">
