...
import { CheckCircle2, XCircle, MousePointerClick, ShieldCheck, Ticket } from 'lucide-react';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'seo.carlBotAlt' });
  ...
  return {
    ...
    alternates: {
      canonical: '/alternatives/carl-bot',
    },
    openGraph: {
      title: t('metaTitle'),
      description: t('metaDescription'),
      url: `${baseUrl}/alternatives/carl-bot`,
    },
  };
}

export default async function CarlBotAlternativePage() {
  const t = await getTranslations('seo.carlBotAlt');
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-800/20">
                  ...
                  <th className="p-6 text-zinc-400 font-medium text-center w-1/3">Carl-bot</th>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="p-8 rounded-3xl bg-gradient-to-b from-zinc-800/50 to-zinc-900/50 border border-zinc-700/50 hover:border-emerald-500/50 transition-all hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(16,185,129,0.1)] group">
              <div className="w-14 h-14 rounded-2xl bg-emerald-500/10 flex items-center justify-center mb-6 group-hover:bg-emerald-500/20 transition-colors">
                <MousePointerClick className="w-7 h-7 text-emerald-400" />
              </div>
