...
import { Check, X, LayoutDashboard, History, Wallet, ArrowRight } from 'lucide-react';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'seo.dynoAlt' });
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://pegasusbot.app';

  return {
    title: t('metaTitle'),
    description: t('metaDescription'),
    alternates: { canonical: '/alternatives/dyno' },
    openGraph: { title: t('metaTitle'), description: t('metaDescription'), url: `${baseUrl}/alternatives/dyno` },
  };
}

export default async function DynoAlternativePage() {
  const t = await getTranslations('seo.dynoAlt');
      ...
      <div className="relative min-h-screen bg-background pt-48 pb-32 overflow-hidden selection:bg-foreground selection:text-background">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-24 relative z-10">
          <Breadcrumbs items={[
            ...
            { name: 'Dyno Alternative', url: '/alternatives/dyno' }
          ]} />
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="p-6 text-foreground/30 text-xs font-medium uppercase tracking-[0.3em] text-center w-1/3">Dyno</th>
                ...
                <tbody>
                  {[
                    ['Dashboard Speed', 'Average', 'Lightning Fast'],
                    ['Economy System', false, true],
                    ['Ticket System', false, true],
                    ['Giveaways', false, true],
                    ['Reaction Roles', 'Limited', 'Unlimited'],
                    ['Auto-Moderation', 'Basic', 'Advanced UI']
                  ].map((row, i) => (
          ...
          <div className="mb-32">
            ...
            <div className="grid md:grid-cols-3 gap-px bg-foreground/10">
              {[
                { icon: LayoutDashboard, titleKey: 'feature1.title', descKey: 'feature1.desc' },
                { icon: History, titleKey: 'feature2.title', descKey: 'feature2.desc' },
                { icon: Wallet, titleKey: 'feature3.title', descKey: 'feature3.desc' },
              ].map((f) => (
                <div key={f.titleKey} className="bg-[#050505] p-8 hover:bg-foreground/[0.02] transition-colors group">
                  <f.icon className="w-6 h-6 text-foreground/30 group-hover:text-foreground transition-colors mb-6" />
                  <h3 className="text-lg font-medium text-foreground mb-3 uppercase tracking-[0.1em]">{t(f.titleKey)}</h3>
                  <p className="text-foreground/40 text-sm leading-relaxed font-light">{t(f.descKey)}</p>
                </div>
              ))}
            </div>
          </div>

          <div className="border border-border bg-[#050505] p-12">
