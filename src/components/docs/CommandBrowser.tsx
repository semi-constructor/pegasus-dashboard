...
export default function CommandBrowser({ categories }: { categories: CommandCategory[] }) {
 ...
 {t('noCommandsFound', { query: searchQuery, category: activeCategory })}
 </div>
 )}
...
function CommandCard({ command }: { command: CommandItem }) {
 ...
 {t('subcommands', { count: command.subcommands?.length })}
 </span>
 )}
 ...
 <div>
 <h5 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3">{t('options')}</h5>
...
function ParamRow({ param }: { param: CommandParam }) {
 ...
 {param.required ? (
 <span className="text-[10px] bg-destructive/10 text-destructive px-1.5 py-0.5 rounded uppercase font-bold tracking-wider border border-destructive/20">{t('required')}</span>
 ) : (
 <span className="text-[10px] bg-secondary text-muted-foreground px-1.5 py-0.5 rounded uppercase font-bold tracking-wider border border-border/50">{t('optional')}</span>
 )}
 </div>
